...
export default function PublicQuiz() {
  ...
  const checkQuizAccess = async (quiz) => {
    ...
    // Check if already attempted (for both logged in and non-logged in users)
    if (attemptedQuizzes.includes(quiz._id)) {
      toast.error("আপনি ইতিমধ্যে এই কুইজে অংশগ্রহণ করেছেন");
      return false;
    }

    // For logged-in users, also check on server
    if (token) {
      try {
        const response = await coreAxios.get(
          `/quizzes/${quiz._id}/check-user`,
          {
        ...
        toast.error("কুইজ চেক করতে সমস্যা হয়েছে");
        return false;
      }
    }

    // For non-logged in users, we'll check phone number later
    return null; // Return null to indicate we need phone verification
  };
  ...
  const onUserInfoSubmit = async (values) => {
    try {
      // First check if phone exists in this quiz
      const response = await coreAxios.get(
        `/quizzes/${selectedQuiz._id}/check-phone?phone=${values.phone}`
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 lg:px-8 py-6">
        ...
        {userInfo && (
          <Alert
            message={`স্বাগতম, ${userInfo.name}! এখন আপনি কুইজে অংশগ্রহণ করতে পারেন।`}
            type="success"
            showIcon
            className="mb-6"
          />
