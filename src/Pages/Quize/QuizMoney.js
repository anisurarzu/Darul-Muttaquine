...
import UpdateQuizeStatus from "./UpdateQuizeStatus";
import CreateQuiz from "./CreateQuize";
import { formatDate } from "../../utilities/dateFormate";
import TableData from "../../components/TableData";
import { coreAxios } from "../../utilities/axios";
import InsertQuizMoney from "./InsertQuizMoney";
...
const QuizMoney = () => {
  ...
  const fetchQuizeInfo = async () => {
    try {
      setLoading(true);
      const response = await coreAxios.get("/quiz-money");
  ...
  const handleDelete = async (RollID) => {
    ...
    try {
      setLoading(true);
      const response = await coreAxios.delete(`/quiz-money/${RollID}`);
      if (response.data) {
  ...
  return (
    <>
      ...
      <Modal
        ...
        width={400}>
        <InsertQuizMoney handleCancel={handleCancel} />
      </Modal>
      <Modal
        title="Please Provided Valid Information"
        open={isModalOpen2}
