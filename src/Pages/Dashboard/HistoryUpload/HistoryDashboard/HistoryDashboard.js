...
const HistoryDashboard = () => {
  ...
  const fetchHistoryDashboardInfo = async () => {
    try {
      setLoading(true);
  ...
  const handleDelete = async (RollID) => {
    try {
      setLoading(true);
      const response = await coreAxios.delete(`/history-info/${RollID}`);
      if (response.data) {
        setLoading(false);
        fetchHistoryDashboardInfo();
      } else {
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="text-sm mx-8 my-6">
          <div className="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-2 border border-tableBorder bg-white px-4 py-5">
            ...
            <div>
              <h3 className="text-[17px]">History Information</h3>
            </div>
            ...
            <table className="w-full text-xl text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {filteredRolls.map((roll) => (
                  <tr key={roll?.HistoryDashboardRollNumber}>
      ...
      <Modal
        ...
        width={800}>
        <HistoryUpload handleCancel={handleCancel} />
      </Modal>
    </>
  );
};
